...
class OrderController {
  constructor() { this.index = this.index.bind(this); this.show = this.show.bind(this); this.update = this.update.bind(this); }
  ...
  async store(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const order = await Order.create({ companyId: user.companyId, peopleId: person.id, paymentMethodId: data.paymentMethodId || null, deliveryTypeId: data.deliveryTypeId || null, statusId: 1, subtotal, shippingValue, discountValue, total, address: data.address || null, note: data.note || null, pointGenerated: data.generatePoints ? Math.floor(total) : 0 });
