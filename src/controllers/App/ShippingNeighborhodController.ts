...
const validateForm = async (data: IShippingNeighborhodItem, id: number | null = null) => {
...
class ShippingNeighborhodController {
  constructor() {
  ...
  async index (req: Request, res: Response): Promise<Response> {
    try {
      ...
      const items = await ShippingNeighborhod.paginate(user.companyId, {
        page,
        where: where,
        order: order,
  ...
  async store (req: Request, res: Response): Promise<Response> {
    try {
      const user = req.user as IReqUser;
      const data = req.body as IShippingNeighborhodItem;
      ...
      const item = await ShippingNeighborhod.create(data);
  ...
  async destroy (req: Request, res: Response): Promise<Response> {
    try {
      ...
      for (const id of ids.split(';')) {
        await ShippingNeighborhod.destroy({ where: { id } });
      }
...
export default new ShippingNeighborhodController();
