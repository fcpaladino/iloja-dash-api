...
const validateForm = async (data: IShippingRuleItem, id: number | null = null) => {
...
class ShippingRuleController {
  ...
  async index (req: Request, res: Response): Promise<Response> {
    try {
      ...
      const items = await ShippingRule.paginate(user.companyId, {
        page,
        where: where,
        order: order,
  ...
  async lists (req: Request, res: Response): Promise<Response> {
    try {
      ...
      const items = await ShippingRule.findAll({
  ...
  async show(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const item = await ShippingRule.findByPk(id, {
        attributes: ['id', 'name', 'active'],
      });
  ...
  async store (req: Request, res: Response): Promise<Response> {
    try {
      const user = req.user as IReqUser;
      const data = req.body as IShippingRuleItem;
      ...
      const item = await ShippingRule.create(data);
  ...
  async destroy (req: Request, res: Response): Promise<Response> {
    try {
      ...
      for (const id of ids.split(';')) {
        await ShippingRule.destroy({ where: { id } });
      }
...
export default new ShippingRuleController();
