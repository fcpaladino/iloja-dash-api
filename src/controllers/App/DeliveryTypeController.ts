...
const validateForm = async (data: IDeliveryTypeItem, id: number | null = null) => {
...
class DeliveryTypeController {
  ...
  async index (req: Request, res: Response): Promise<Response> {
    try {
      ...
      const items = await DeliveryType.paginate(user.companyId, {
        page,
        where: where,
        order: order,
  ...
  async lists (req: Request, res: Response): Promise<Response> {
    try {
      ...
      const items = await DeliveryType.findAll({
  ...
  async show(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const item = await DeliveryType.findByPk(id, {
  ...
  async store (req: Request, res: Response): Promise<Response> {
    try {
      const user = req.user as IReqUser;
      const data = req.body as IDeliveryTypeItem;
      ...
      const item = await DeliveryType.create(data);
