...
const validateForm = async (data: IShippingCepItem, id: number | null = null) => {
...
class ShippingCepController {
  ...
  async index (req: Request, res: Response): Promise<Response> {
    try {
      ...
      const items = await ShippingCep.paginate(user.companyId, {
        page,
        where: where,
        order: order,
        attributes: ['id', 'startCep', 'endCep', 'price'],
      });
  ...
  async lists (req: Request, res: Response): Promise<Response> {
    try {
      ...
      const items = await ShippingCep.findAll({
  ...
  async show(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const item = await ShippingCep.findByPk(id, {
        attributes: ['id', 'startCep', 'endCep', 'price'],
      });
  ...
  async store (req: Request, res: Response): Promise<Response> {
    try {
      const user = req.user as IReqUser;
      const data = req.body as IShippingCepItem;
      ...
      const item = await ShippingCep.create(data);
  ...
  async destroy (req: Request, res: Response): Promise<Response> {
    try {
      ...
      for (const id of ids.split(';')) {
        await ShippingCep.destroy({ where: { id } });
      }
...
export default new ShippingCepController();
